...
// Minimal SHA-256 for disk-cache user message hashing (public domain)
...
#define _s0(x) (rotr32(x, 7) ^ rotr32(x, 18) ^ ((x) >> 3))
#define _s1(x) (rotr32(x, 17) ^ rotr32(x, 19) ^ ((x) >> 10))
#define _Ch(x,y,z) (z^(x&(y^z)))
#define _Maj(x,y,z) ((x&y)|(z&(x|y)))
...
static inline void sha256_process_block(sha256_ctx *ctx) {
    uint32_t W[64];
    ...
    for (unsigned j = 0; j < 64; j++) {
        uint32_t t1 = h + _S1(e) + _Ch(e,f,g) + SHA256_K[j] + W[j];
...
static inline void sha256_update(sha256_ctx *ctx, const uint8_t *data, size_t len) {
    ...
    while (len > 0) {
        ...
        ctx->count++;
        len--;
        if (pos == 64) { pos = 0; sha256_process_block(ctx); }
    }
}

static inline void sha256_final(sha256_ctx *ctx, uint8_t *digest) {
    uint64_t lenBits = ctx->count << 3;
    uint32_t pos = (uint32_t)ctx->count & 0x3F;
    ctx->buffer[pos++] = 0x80;
    while (pos != 56) { pos &= 0x3F; if (pos == 0) sha256_process_block(ctx); ctx->buffer[pos++] = 0; }
    for (unsigned i = 0; i < 8; i++) { ctx->buffer[pos++] = (uint8_t)(lenBits >> 56); lenBits <<= 8; }
