...
class ServerProcess:
    ...
    auto_disk_cache: bool | None = False
    auto_disk_cache_max: int | None = None
    ...
    def start(self, timeout_seconds: int = DEFAULT_HTTP_TIMEOUT) -> None:
        ...
        if self.auto_disk_cache:
            server_args.append("--auto-disk-cache")
        if self.auto_disk_cache_max is not None:
            server_args.extend(["--auto-disk-cache-max", self.auto_disk_cache_max])
        if self.n_ga:
